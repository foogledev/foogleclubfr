...
function Library:MakeDraggable(ClickObject, Object)
	local Dragging = nil
	local DragInput = nil
	local DragStart = nil
...
function Library:CreateWindow(Config)
	...
	Topbar.WindowName.Text = Config.WindowName .. " " .. Config.Version
	...
	RunService.RenderStepped:Connect(function()
		if Library and Library.Toggle and Screen and Screen:FindFirstChild('ToolTip') then
		...
		function TabInit:CreateSection(Name)
			...
			function SectionInit:CreateSlider(Name, Min, Max, Default, Precise, Callback)
				...
				local function Sliding(Input)
					local Position = UDim2.new(math.clamp((Input.Position.X - Slider.Slider.AbsolutePosition.X) /
						Slider.Slider.AbsoluteSize.X, 0, 1), 0, 1, 0)
			...
			function SectionInit:CreateDropdown(Name, OptionTable, InitialValue, Callback)
				local DropdownInit = {}
				...
				Dropdown.MouseButton1Click:Connect(function()
					...
					if DropdownToggle then
						Dropdown.Size = UDim2.new(1, -10, 0,
						Dropdown.Container.Holder.Container.ListLayout.AbsoluteContentSize.Y + Dropdown.Title.TextBounds.Y + 30)
				...
				function DropdownInit:ReloadTable(Table)
					Dropdown.Container.Holder.Visible = false
					for _, OptionName in pairs(Table) do
						...
						Dropdown.Container.Holder.Size = UDim2.new(1, -5, 0,
				...
				DropdownInit:ReloadTable(OptionTable)
				...
				function DropdownInit:RefreshOptions(Table)
					-- refreshes the current table
					DropdownInit:ClearOptions()
					DropdownInit:ReloadTable(Table)
				end
				...
				Colorpicker.MouseButton1Click:Connect(function()
					if not Pallete.Visible then
						ColorpickerRender = RunService.RenderStepped:Connect(function()
							Pallete.Position = UDim2.new(0, Colorpicker.Color.AbsolutePosition.X - 129, 0,
				...
				Pallete.GradientPallete.InputBegan:Connect(function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						ColorRender = RunService.RenderStepped:Connect(function()
							...
							local ColorX = math.clamp(Mouse.X - Pallete.GradientPallete.AbsolutePosition.X, 0,
								Pallete.GradientPallete.AbsoluteSize.X) / Pallete.GradientPallete.AbsoluteSize.X
							local ColorY = math.clamp((Mouse.Y - 37) - Pallete.GradientPallete.AbsolutePosition.Y, 0,
								Pallete.GradientPallete.AbsoluteSize.Y) / Pallete.GradientPallete.AbsoluteSize.Y
				...
				Pallete.ColorSlider.InputBegan:Connect(function(Input)
					if Input.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						HueRender = RunService.RenderStepped:Connect(function()
							local Mouse = UserInputService:GetMouseLocation()
							local HueX = math.clamp(Mouse.X - Pallete.ColorSlider.AbsolutePosition.X, 0, Pallete.ColorSlider.AbsoluteSize.X) /
